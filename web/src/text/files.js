...
const PATH = new RegExp(`(^|[\\s(\`])((?:/(?:[\\w.-]+/)*[\\w.-]*\\.(?:${EXT}))|(?:\\.{1,2}/)?(?:[\\w.-]+/)*[\\w.-]*\\.(?:${EXT}))(?=[\\s).,;:\`]|$)`, "g");
const ICON =
...
register((text, context) =>
    text.replace(PATH, (whole, before, path) =>
        isFile(path)
            ? `${before}<a class="row-pill file-pill" href="#/${context.env}/file?q=${encodeURIComponent(path)}">${ICON}${path.startsWith("/") ? path.split("/").pop() : path}</a>`
            : whole
    )
);
