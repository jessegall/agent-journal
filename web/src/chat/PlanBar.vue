...
const done = computed(() => (plan.value ? plan.value.data.phases.filter((p) => p.todos.length && p.todos.every((n) => (rows("todo").find((t) => t.n === n) || {}).completed)).length : 0));
</script>
...
<style scoped>
.planbar { display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px 22px; border: 0; border-bottom: 1px solid var(--border); background: var(--raised); color: var(--text-2); text-align: left; cursor: pointer; }
.planbar:hover { background: var(--hover); }
.ptitle { color: var(--text); font-weight: 500; }
.wait { color: var(--warn); }
.grow { flex: 1; }
