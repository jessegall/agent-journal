...
async function send() {
    ...
    try {
        ...
        await create(route.value.env, "message", {
            title,
            brief: text.value.trim(),
            ...(kind.value === "transcript" ? {kind: "transcript"} : {}),
        });
        text.value = "";
        await reload();
    } catch (e) {
    ...
    <div class="chat">
        ...
        <form class="composer" @submit.prevent="send">
            <textarea
                v-model="text"
                rows="3"
                :placeholder="kind === 'transcript' ? 'Paste the transcript…' : 'Write to the agent…'"
            ...
            <div class="tools">
                <button
                    type="button"
                    :class="['flat', {on: kind === 'transcript'}]"
                    @click="kind = kind === 'transcript' ? 'message' : 'transcript'"
                >
                    <Icon name="clip" :size="14" />
                    transcript
                </button>
                <span class="error">{{ error }}</span>
                <Btn kind="primary" small @click="send">
