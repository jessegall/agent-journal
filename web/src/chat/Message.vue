...
const reactions = computed(() => {
    const faces = {};
    for (const r of rows("reaction").filter((r) => r.refs.includes(props.message.ref) && !r.deleted))
        (faces[r.data.face] ||= []).push(r.seen[0]);
...
<template>
    <article :class="['message', by]">
        <header class="head">
            ...
            <span v-if="message.completed" class="state">
        ...
        <div v-for="s in message.sections" :key="s.title" class="part">
            <span class="pwords">“{{ s.title }}”</span>
            <span class="became">→ {{ s.body }}</span>
        </div>
        ...
        <footer class="foot">
            <button
                v-for="[face, who] in reactions"
                :key="face"
                type="button"
                :class="['face', {mine: who.includes('user')}]"
                :title="who.join(', ')"
                @click="react(face)"
            >
                {{ face }} {{ who.length }}
            ...
            <span v-if="picking" class="picker">
                <button v-for="f in FACES" :key="f" type="button" class="pick" @click="react(f)">{{ f }}</button>
            </span>
        </footer>
    </article>
</template>
