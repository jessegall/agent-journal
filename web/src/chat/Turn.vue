...
import {clock, reload, rows} from "../store.js";
...
<template>
    <div :class="['thread-turn', {mine, ask: turn.type === 'question'}]" @mouseleave="picking = false">
        <div class="thread-bubble md">
            ...
            <div class="thread-text">{{ turn.brief || turn.title }}</div>
            <template v-for="s in turn.sections" :key="s.title">
                <div class="thread-became">
                    <span class="thread-became-dot" />
                    <span class="thread-became-word">“{{ s.title }}” → {{ s.body }}</span>
                </div>
            </template>
                ...
                <div class="thread-files">
                    <template v-for="f in files" :key="f">
                        <a class="thread-file" :href="fileUrl(route.env, turn.type, turn.n, f)" target="_blank">
                            <template v-if="/\.(png|jpe?g|gif|webp)$/i.test(f)">
