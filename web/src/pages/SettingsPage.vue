  ...
  <section class="settings">
    ...
    <div v-for="f in features" :key="f.name" class="feature">
      ...
      <span class="ftext">
        ...
        <span v-if="Object.keys(f.trigger).length" class="ftrig">{{ f.trigger.on ? `on ${f.trigger.on}` : f.trigger.at ? `at ${f.trigger.at.join(", ")} percent` : `every ${f.trigger.every} ${f.trigger.unit}` }}</span>
      </span>
    </div>
    <h2>Keep</h2>
    <div v-for="type in ['report', 'todo']" :key="type" class="keep">
      <span class="ktext">{{ type }}s are archived after</span>
      <input v-model="days[type]" type="number" min="0" :placeholder="String(keep[type] ?? (type === 'report' ? 14 : 7))" @change="setKeep(type)">
...
<style scoped>
.settings { max-width: 720px; padding: 22px 28px 60px; }
h2 { margin: 22px 0 4px; font-size: 15px; font-weight: 600; }
.lead { margin: 0 0 12px; color: var(--text-3); }
.feature { display: flex; align-items: flex-start; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--border); }
.ftext { display: flex; flex-direction: column; }
.ftitle { font-weight: 500; }
.fabs { color: var(--text-2); }
.ftrig { color: var(--text-3); font-size: 12px; }
.keep { display: flex; align-items: center; gap: 8px; padding: 6px 0; color: var(--text-2); }
.keep input { width: 64px; padding: 4px 8px; border: 1px solid var(--border-2); border-radius: 6px; background: var(--raised); }
.env { display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border); }
</style>
