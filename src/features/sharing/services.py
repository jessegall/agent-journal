...
from engine.services import allocate, files_for
...
def share_services(root: Path, taken: set) -> list:
    ...
    specs = [ServiceSpec(id=SERVER, plugin="sharing", service="server", run=[*entry("features.sharing.server"), str(root), str(port)],
                         cwd=str(Path(root).parent), port=port, blocked=blocked, url=f"http://127.0.0.1:{port}", **files_for(root, SERVER))]
    command = tunler()
    if command:
        specs.append(ServiceSpec(id=TUNNEL, plugin="sharing", service="tunnel", cwd=str(Path(root).parent),
                                 run=[command, str(port), f"--domain={subdomain(root)}", "--inspect=0"], **files_for(root, TUNNEL)))
    return specs
