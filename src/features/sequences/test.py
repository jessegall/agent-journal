...
def test_a_sequence_hands_its_steps_one_at_a_time_and_starts_on_its_moment():
    ...
    assert (ship(record), ship(record)) == (["Filing a dump", "Building a plan", "Working a card from the board", "Revising the board's drafts", "Building a board from a document", "Drafting tickets from a document", "Writing an update"], []), "shipped once, never twice"
    ...
    assert [label for label, _ in marks] == ["Sequence started", "Sequence moved on", "Sequence moved on", "Sequence finished"] and {c for _, c in marks} == {"#a78bfa"}, \
        f"the chat shows a violet mark as it starts, moves on and finishes: {marks}"
