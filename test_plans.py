...
code, out = j("plans", "auto", "3", "on")
check("an agent cannot switch a plan's auto mode", (code, "only the user switches" in out), (1, True))
check("the user switches it on", plans.set_auto(root, 3, True, AT, source="web", track="default")[0], True)
_saved = plans._all(root, "default")[2]["phases"][0].pop("continued_at")
check("in auto mode a completed checkpoint does not hold the plan", (plans.stall(root, "default"), [t["n"] for t in todo.ready(root, "default")]), ("", [6, 7]))
check("the row says it is in auto mode", show(3)["auto"], True)
plans.set_auto(root, 3, False, AT, source="web", track="default")
