...
# the extension's own origin is not a foreign page: checked against a write that changes nothing (an unknown environment is 404, not 403)
status, got = post("/api/env/nowhere-at-all/messages", {"text": "from the extension"}, headers={"Origin": "chrome-extension://abcdefghijklmnop"})
check("a write from a browser extension's own origin is not refused as foreign", status, 404)
...
# WHETHER THE SESSION HAS A SEAT. The launcher stamps its session every look; a stamp gone
# quiet means plain `claude` started it and nothing written in the viewer reaches it while idle.
state.put(root, "seat_seen", int(time.time()), stem=_run_stem)
check("a session the launcher stamped just now has a seat", _act.channel_now(root, _run_stem), True)
state.put(root, "seat_seen", int(time.time()) - 120, stem=_run_stem)
...
check("an older session with no stamp has no seat", _act.channel_now(root, _run_stem), False)
