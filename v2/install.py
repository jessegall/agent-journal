...
from v2.migrate import migrate  # noqa: E402
from v2.providers import PROVIDERS  # noqa: E402
from v2.skills import write as write_skills  # noqa: E402
...
def upgrade(project: Path, root: Path | None = None) -> list[str]:
    ...
    moved = migrate(root)
    done.append(f"record migrated: {len(moved)} resources" if moved else "record already in v2's shape")
    return done
