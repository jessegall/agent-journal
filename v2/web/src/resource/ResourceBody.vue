...
<template>
  <article class="body">
    <header class="top">
      <span class="kind"><Icon :name="kind.icon" :size="13" /> {{ kind.title }} {{ resource.n }}</span>
    ...
    <section v-if="resource.completed" class="block">
      <h3>{{ label(resource.type, "outcome", word(resource.type, "complete").replace(/^\w/, (c) => c.toUpperCase())) }}</h3>
      <div class="text">{{ resource.outcome || age(resource.completed) }}</div>
    </section>
    <section v-if="files.length" class="block">
      <h3>Files</h3>
      <a v-for="[name, what] in files" :key="name" class="file" :href="fileUrl(route.env, resource.type, resource.n, name)" target="_blank"><Icon name="clip" :size="13" /> {{ name }}<span v-if="what" class="what"> — {{ what }}</span></a>
    </section>
...
<style scoped>
.body { padding: 16px 20px 30px; }
.top { display: flex; align-items: center; gap: 10px; color: var(--text-3); font-size: 11.5px; text-transform: uppercase; letter-spacing: .04em; }
.kind { display: inline-flex; align-items: center; gap: 6px; color: var(--accent-text); }
.age { flex: 1; }
.title { margin: 10px 0 4px; font-size: 19px; font-weight: 600; line-height: 1.3; }
.abstract { margin: 0 0 8px; color: var(--text-2); }
.block { margin-top: 16px; }
.block h3 { margin: 0 0 4px; font-size: 12px; font-weight: 600; color: var(--text-3); text-transform: uppercase; letter-spacing: .04em; }
.text { white-space: pre-wrap; color: var(--text-2); }
.file { display: flex; align-items: center; gap: 6px; color: var(--accent-text); }
.what { color: var(--text-3); }
.foot { margin-top: 22px; color: var(--text-3); font-size: 11.5px; }
</style>
