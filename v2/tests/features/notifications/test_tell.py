...
from v2.engine.actors import User  # noqa: E402
...
CONTROLLERS["message"](record, actor=AGENT).complete(1, "read")
...
CONTROLLERS["todo"](record, actor=AGENT).create("a chore")
check("a to-do added by the agent: the type's word for create", told(record)[-1][0], "To-do 1 add")
CONTROLLERS["pin"](record, actor=SYSTEM).create("bookkeeping")
check("bookkeeping by the system makes none", len(told(record)), 2)
CONTROLLERS["nudge"](record, actor=AGENT).create("a nudge")
CONTROLLERS["agent"](record, actor=AGENT).by_session("s")
...
CONTROLLERS["notification"](record, actor=AGENT).create("the migration is through", about="todo:1")
