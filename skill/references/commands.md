...
**The viewer**
    ...
    journal codex [prompt]           start Codex under the journal's launcher, the same way; the journal's hooks are wired into .codex/hooks.json and its skills into .agents/skills first, and Codex asks once to trust the hooks (/hooks). The viewer names the session Codex and reads its rollout file. --quiet never types, --dry-run shows the command; any other flag is passed through to codex
