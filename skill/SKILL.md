---
name: journal
description: "The project's journal, core skill: how to file what happens so it survives a compaction and reaches every later session — the tag on each message, declared work, the first decision on every request (the current work, a to-do, or work to do now), choosing the environment, looking before you answer, and what to do at a start, after a compaction or when a hook holds you. Use it whenever the user asks for a feature, a fix or any piece of work; whenever a hook holds your stop or denies a tool call; before your first declaration or search in a session; when the user says later, not yet, or after X; when you are about to say 'I think we decided…'; and at a fresh start or after a compaction. Load it even when the request seems small. The detail for to-dos, questions, messages, pins and reminders, docs and reports, and subagents is in the journal-todos, journal-questions, journal-messages, journal-memory, journal-docs and journal-agents skills. Not for subagents: a subagent reports what it found and the main conversation files it."
---
