...
class InboxController(Controller):
    ...
    def index(self, root: Path, p: Payload) -> Result:
        ...
        if p.text("sort"):
            ...
            query = repo.query().order_by("n", p.text("order") or fmt.DESC).order_by("status_order")
    ...
    def store(self, root: Path, p: Payload) -> Result:
        outcome = inbox.add(root, str(p.get("text") or ""), p.at, source=p.source, track=p.env or None)
    ...
    def update(self, root: Path, p: Payload) -> Result:
        return Result.of(inbox.update(root, p.id, str(p.get("text") or ""), p.env or None), self._row(root, p, p.id))

    def process(self, root: Path, p: Payload) -> Result:
        became = p.get("became") or []
        became = [became] if isinstance(became, str) else list(became)
        return Result.of(inbox.process(root, p.id, p.text("part"), became, p.at, p.env or None), self._row(root, p, p.id))
    ...
    def move(self, root: Path, p: Payload) -> Result:
        return Result.of(inbox.move(root, p.id, p.text("environment"), p.at, p.env or None), self._row(root, p, p.id))
