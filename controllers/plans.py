...
class PlansController(Controller):
    resource = "plans"
    noun = "plan"
    actions = ("index", "show", "store", "fromdoc", "phase", "todos", "activate", "proceed", "acknowledge", "park", "destroy", "link")
    numbered = ("show", "phase", "todos", "activate", "proceed", "acknowledge", "park", "destroy", "link")
    ...
    def store(self, root: Path, p: plan_payloads.StorePayload) -> Result:
        outcome = plans.add(root, p.title, p.goal, p.body, p.at, source=p.source, track=p.env or None)
