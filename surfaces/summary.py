...
def environment(record: Record) -> dict:
    ...
    return {
        ...
        "work": work(current),
        "last": work(last),
        "plans": [plan(p, todos) for p in Plans(record, actor=SYSTEM)._every() if p.status in SHOWN and not p.completed],
        "auto": switched(record, "work.auto"),
        "counts": {
            "messages": len(Messages(record, actor=SYSTEM).unread(USER)),
            "questions": len([q for q in Questions(record, actor=SYSTEM)._every() if not q.completed]),
            "todos": len([n for n, done in todos.items() if not done]),
            "suggestions": len([s for s in Suggestions(record, actor=SYSTEM)._every() if not s.completed]),
        },
    }
...
def summarize(root: Path) -> dict:
    m = manifest(root)
    names = dict.fromkeys([m["environment"], *(e.title for e in Environments(Record(root, m["environment"]), actor=SYSTEM)._every() if not e.completed)])
