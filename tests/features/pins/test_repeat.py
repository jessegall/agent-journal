...
check("said at the first tenth", (nudges(record), CONTROLLERS["nudge"](record).load(1).brief), (["2 pins standing, read them"], "1. the hook payload carries the parent's session id; 2. tests run bounded"))
...
check("superseded: the old is struck, saying by which, and the new links it", (pins.load(1).outcome, newer.refs), ("superseded by pin 3", ["pin:1"]))
rule = pins.promote(2)
...
CONTROLLERS["reminder"](record, actor=USER).create("run the suites first", until="the suites are green on CI")
check("a reminder keeps its until", CONTROLLERS["reminder"](record).load(1).data["until"], "the suites are green on CI")
