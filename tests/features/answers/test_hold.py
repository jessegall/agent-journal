...
record = fresh("main")
questions = features.FEATURES["answers"]
...
record.set_setting("answers", {"hold": 8})
check("a setting says how long instead", questions.held_for(record), 8)

got = dispatch("GET", "/api/main/settings", record.root, {}, {})
check("the viewer is handed the hold with the rest of the settings", (got.code, got.body["answers"]["hold"]), (200, 8))
saved = dispatch("POST", "/api/main/settings", record.root, {}, {"answers": {"hold": 1}})
check("and can set it", (saved.code, saved.body["answers"]["hold"]), (200, 1))
