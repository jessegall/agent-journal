...
from features.questions.feature import offers_choices  # noqa: E402
...
check("choices in prose: the agent is told once, and its writes are held", ([n for n in nudges(record) if "choices in prose" in n], bool(holds().get("questions.asking"))), (["your last message offers choices in prose"], True))
...
check("a question asked properly lifts the hold", holds().get("questions.asking", ""), "")
