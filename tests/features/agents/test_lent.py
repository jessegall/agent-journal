...
from controllers.types import Agents, Nudges, Todos, Works  # noqa: E402
...
check("report without --agent is refused", refused(lambda: Todos(record, actor=AGENT).report(row.n, "x")), "report is a subagent's word — the dispatcher closes a row with done")
...
check("a silent subagent's row is back on the list, and the dispatcher told which", (todos.load(row.n).data.get("assigned"), todos.load(row.n).data.get("lapsed"), any("went silent" in t for t in [n.title for n in Nudges(record).all()])), ("", "runner-1", True))
