...
idle(record)
idle(record)
check("open work with an empty log is said at each idle, asking for the log", nudges(record), ["work 1 is still open, with nothing logged"] * 2)
works.action("log")("Chose the header, because the footer waits on it")
idle(record)
check("once logged, the open work is said with what to do about it", nudges(record)[-1], "work 1 is still open")
