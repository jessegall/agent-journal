...
from surfaces.updates import announce, newer  # noqa: E402
...
record = fresh("main")
check("a first install only notes its version", (announce(record.root, "2.10.0"), Notifications(record).all()), ("", []))
check("the same version again says nothing", announce(record.root, "2.10.0"), "")
check("a new version is announced", announce(record.root, "2.11.0"), "2.11.0")
...
check("and only once", (announce(record.root, "2.11.0"), len(Notifications(record).all())), ("", 1))
