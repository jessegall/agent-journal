...
t = Translator(56)
check("a cursor address moves down by the band", t.feed(b"\x1b[5;10H"), b"\x1b[8;10H")
check("home is the first row under the band", t.feed(b"\x1b[H"), b"\x1b[%d;1H" % (ROWS + 1))
check("a scroll region moves down too", t.feed(b"\x1b[2;53r"), b"\x1b[5;56r")
check("a reset region is the whole screen under the band", t.feed(b"\x1b[r"), b"\x1b[%d;56r" % (ROWS + 1))
check("a vertical position moves down", t.feed(b"\x1b[12d"), b"\x1b[15d")
check("a sequence split across reads is held and joined", (t.feed(b"abc\x1b[3"), t.feed(b";4Hdef")), (b"abc", b"\x1b[6;4Hdef"))
