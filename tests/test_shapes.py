...
def test_every_type_is_shaped_fields_and_labels_gathered_from_its_mixins():
    ...
    assert reasoned == ["pin", "rule", "style"], "pins, rules and style rules share one reasoning shape"
    assert TYPES["pin"].labels == {"brief": "Reasoning", "outcome": "Why struck"}, "the reasoning shape names the brief and the strike"
    assert (TYPES["pin"].labels == TYPES["rule"].labels) is True, "a shape declared once is the same object on both"
    ...
    assert m["types"]["pin"]["labels"] == {"brief": "Reasoning", "outcome": "Why struck"}, "the manifest says a pin's labels"
