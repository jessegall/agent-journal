...
# a report and a plan are commented on the way a to-do is — and checked, not waved through
P.cli("reports", "add", "what the loader does today", "--brief", stdin="the shape of it")
j("plans", "add", "Rework the loader", "--goal=one loader, one path")
code, out = j("comments", "add", "report 1", "this answers it, thank you")
check("a comment can be about a report", (code, "on report 1" in out), (0, True))
code, out = j("comments", "add", "plan 1", "split phase 2 in two")
check("and about a plan", (code, "on plan 1" in out), (0, True))
code, out = j("comments", "add", "report 9", "nothing there")
check("a report that is not there is refused", (code, "no report 9" in out), (1, True))
code, out = j("comments", "add", "plan 9", "nothing there")
check("nor a plan that is not there", (code, "no plan 9" in out), (1, True))
...
check("the stop names every new comment and what it is about",
      ...
      ("the user left 4 comments", True, True, True))
