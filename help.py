...
GROUPS: dict[str, tuple[str, ...]] = {
    ...
    "pins": (
        ...
        'journal pins add "<claim>" --brief    the reasoning on stdin; pins take a long form too',
        "journal pins show N           THE PIN: the claim and its reasoning; `pins N --full` is the conversation",
        'journal pins amend N "<section title>" --brief   append a section; `pins replace N --brief` swaps it',
    ...
    "rules": (
        ...
        "journal rules [--all] [--order=asc|desc]   every rule, numbered, newest first",
        "journal rules add \"<ruling>\" --brief   the reasoning on stdin: uncapped, never injected, read on demand",
        "journal rules show N          THE RULE: the claim, its reasoning, what it cites",
        "journal rules N --full        the CONVERSATION it was written in — a different question",
    ...
    "grant": (
        ...
        'journal todos report <n> "<how>"    a SUBAGENT saying a row is finished; only the parent closes one',
        "A subagent gets its own ledger at environments/<lent>/agents/<id>/, and is told its own name by the hook on its first tool call — it cannot know it otherwise. It reads the environment's pins and can never write one.",
    ...
    "todos": (
        ...
        'journal todos ask N "<question>"    files a question linked to to-do N; it waits on the user while any linked question is open, and auto moves on to the next',
    ...
    "docs": (
        ...
        'journal docs add "<title>" --abstract="<one line>" [--global] --brief   a new doc; the brief on stdin is its intro',
    ...
    "system": (
        ...
        "journal codex [prompt]          start Codex under the journal's launcher, the same way, with the journal's hooks wired into .codex/hooks.json first (Codex asks once to trust them: /hooks); --quiet never types, --dry-run shows the command",
