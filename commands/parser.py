...
def built(only: str) -> argparse.ArgumentParser:
    ...
    add_query(cmds, "tidy", "run the housekeeping now: trim captures and logs, drop quiet sessions' files", lambda ctx: str(tidy(ctx["record"].root, features.FEATURES["runtime_cleanup"].values(ctx["record"]).days)))
