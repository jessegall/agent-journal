...
import testkit, tools, transcript  # noqa: E402
...
def fire(event, **extra):
    return P.hook(event, session_id="s1", transcript_path=str(path), **extra)[1]


#: `tools run` shells the entry script out with its stdio left INHERITED (tools.py uses
#: `subprocess.call`, uncaptured) — that write lands straight on the harness's own pipe and
#: corrupts the JSON-per-line protocol every other call here relies on. Genuinely a process
#: boundary, so these three calls alone keep spawning a real `journal.py`.
def run_tool(*args):
    p = testkit.spawn(d, *args, env={**os.environ, transcript.SESSION_ENV: "s1"})
...
code, out = run_tool("tools", "run", "mover", "Bone", "RenderNode")
...
code, out = run_tool("tools", "run", "mover", "--dry-run", "X")
...
code, out = run_tool("tools", "run", "callers", "handle")
...
check("a subagent cannot add a tool — a tool is the project's, and it was lent none",
...
check("but may run one", bool(testkit.denied(out)), False)
