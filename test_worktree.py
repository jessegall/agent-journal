...
def git(cwd, *a):
    return subprocess.run(["git", *a], cwd=cwd, capture_output=True, text=True, timeout=180).stdout.strip()
...
subprocess.run([str(main / ".journal" / "journal.py"), "pin", "a fact from main"], env=env, capture_output=True, timeout=180)
...
subprocess.run([str(wt / ".journal" / "journal.py"), "pin", "a fact from the worktree"], env=wenv, capture_output=True, timeout=180)
p = subprocess.run([str(main / ".journal" / "journal.py"), "pins"], env=env, capture_output=True, text=True, timeout=180)
check("a pin written in the worktree is in the main journal", "a fact from the worktree" in p.stdout, True)
p = subprocess.run([str(wt / ".journal" / "journal.py"), "worktree"], env=wenv, capture_output=True, text=True, timeout=180)
...
p = subprocess.run([str(main / ".journal" / "journal.py"), "worktree"], env=env, capture_output=True, text=True, timeout=180)
...
p = subprocess.run([str(wt2 / ".journal" / "journal.py"), "worktree", "link"], env={**os.environ, transcript.SESSION_ENV: "w2"}, capture_output=True, text=True, timeout=180)
...
def ignores(pattern: str, make) -> bool:
    ...
    make(target)
    r = subprocess.run(["git", "check-ignore", "-q", ".journal"], cwd=probe, timeout=180)
...
_p = subprocess.run([sys.executable, str(far / ".journal" / "hook.py")], cwd=str(far),
                    ...
                    env=wt_env, capture_output=True, text=True, timeout=180)
