...
def test_a_slow_request_is_reported_only_when_the_budget_is_on():
    ...
    record = fresh()
    turned(record, False)
    with FEATURES["faults"].reports.watched(record.root, record.env, "request", "GET /api/main/message"):
        time.sleep(0.08)
...
def test_a_fast_request_is_never_reported():
    ...
    record = fresh()
    turned(record, True)
    with FEATURES["faults"].reports.watched(record.root, record.env, "request", "GET /api/main/fact"):
        pass
    assert told(record) == []
...
def test_going_over_again_counts_but_tells_the_agent_once():
    ...
    turned(record, True)
    for _ in range(2):
        with FEATURES["faults"].reports.watched(record.root, record.env, "command", "message all"):
            time.sleep(0.08)
...
def test_what_the_viewer_throws_is_filed_under_the_same_switch():
    ...
    record = fresh()
    turned(record, False)
    assert FEATURES["faults"].reports.heard(record.root, record.env, "agents.some is not a function", "Sidebar.vue", "at r") is False, \
        "switched off, nothing is filed"
    turned(record, True)
    assert FEATURES["faults"].reports.heard(record.root, record.env, "agents.some is not a function", "Sidebar.vue", "at r") is True
