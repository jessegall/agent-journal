...
def test_a_dump_is_read_and_filed_item_by_item_and_closes_when_every_item_is_settled(tmp_path):
    ...
    agent.read(dump.n, "text", "a transcript of Tuesday's planning meeting")
    ...
    assert "has no item 'other.md'" in refused(lambda: agent.read(dump.n, "other.md", "x")), "an item the dump does not have is refused"
