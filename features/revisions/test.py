...
def test_edits_change_the_open_revision_and_a_kept_one_never_changes():
    ...
    assert len(mine.load(doc.n).data["revisions"]) == 1, "while the revision is open, edits change it in place"
    ...
    now = mine.load(doc.n)
    assert (now.title, now.sections, len(now.data["revisions"])) == ("How handlers and events are written", [], 2), "the next edit opens a new revision"
    ...
    assert len(mine.load(doc.n).data["revisions"]) == 3, "an open revision left alone long enough is kept by itself"
